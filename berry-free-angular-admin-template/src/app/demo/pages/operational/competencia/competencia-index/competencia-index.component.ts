...
@Component({
  selector: 'app-competencia-index',
  standalone: true,
  imports: [BreadcrumbComponent, DataTablesModule, BotonesComponent],
  templateUrl: './competencia-index.component.html',
  styleUrl: './competencia-index.component.scss'
})
export class CompetenciaIndexComponent {
  title = "Listado de competencia";
  ...
  nuevo() {
    this.helperService.redirectApp("adashboard/competencia/nuevo");
  }
  ...
  data(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.service.datatable("Competencia", data).subscribe(
        (datos) => {
          resolve(datos);
        },
        (error) => {
  ...
  eliminar(id: number) {
    this.helperService.confirmDelete(() => {
      this.service.delete("Competencia", id).subscribe(
        (response) => {
          if (response.status) {
  ...
  editar(id: number) {
    ...
    this.helperService.redirectApp(`adashboard/competencia/editar/${id}`);
