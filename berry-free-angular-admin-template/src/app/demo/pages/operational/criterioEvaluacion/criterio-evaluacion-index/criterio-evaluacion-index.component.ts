...
@Component({
  selector: 'app-criterio-evaluacion-index',
  standalone: true,
  imports: [BreadcrumbComponent, DataTablesModule, BotonesComponent],
  templateUrl: './criterio-evaluacion-index.component.html',
  styleUrl: './criterio-evaluacion-index.component.scss'
})
export class CriterioEvaluacionIndexComponent {
  ...
  nuevo() {
    this.helperService.redirectApp("adashboard/proyecto/nuevo");
  }
  ...
  data(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.service.datatable("CriterioEvaluacion", data).subscribe(
        (datos) => {
          resolve(datos);
        },
        (error) => {
  ...
  eliminar(id: number) {
    this.helperService.confirmDelete(() => {
      this.service.delete("Proyecto", id).subscribe(
        (response) => {
          if (response.status) {
  ...
  editar(id: number) {
    ...
    this.helperService.redirectApp(`adashboard/proyecto/editar/${id}`);
